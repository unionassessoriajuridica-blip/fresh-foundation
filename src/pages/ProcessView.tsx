    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => navigate(`/novo-processo?edit=${id}`)} className="bg-primary hover:bg-primary/90">
            ...
            <Tabs defaultValue="dados-pessoais" className="w-full">
              ...
              <TabsContent value="dados-pessoais" className="mt-6">
                <div className="space-y-6">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold mb-4">Dados do Processo</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">Número do Processo</label>
                        <p className="text-base font-mono">{processo.numero_processo}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">Tipo do Processo</label>
                        <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20">
                          {processo.tipo_processo}
                        </Badge>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">Cliente Preso</label>
                      ...
                      <div>
                        ...
                        <p className="text-base">{processo.prazo ? formatDate(processo.prazo) : '-'}</p>
                      </div>
                      {processo.descricao && (
                        <div className="md:col-span-2">
                      ...
                      <div className="space-y-4">
                        {observacoes.map((obs, index) => (
                          <Card key={index}>
                            ...
                            <CardContent>
                              <p className="text-sm text-muted-foreground">{obs.conteudo}</p>
                            </CardContent>
                          </Card>
                        ))}
                      </div>
                  ...
                  {financeiro.length > 0 ? (
                    <div className="space-y-4">
                      {['Entrada', 'Honorários', 'TMP'].map(tipo => {
                        ...
                        return (
                          <div key={tipo}>
                            <h4 className="font-medium text-base mb-3">{tipo}</h4>
                    ...
                    <p className="text-muted-foreground">Nenhuma informação financeira cadastrada.</p>
                  )}
                </div>
              </TabsContent>
              ...
              <TabsContent value="responsavel" className="mt-6">
                <div className="space-y-6">
                  <h3 className="text-lg font-semibold">Responsável Financeiro</h3>
                  ...
                  {responsavel ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-base">{formatDate(responsavel.data_nascimento)}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">Telefone</label>
                      ...
                      <div className="md:col-span-2">
                        <label className="text-sm font-medium text-muted-foreground">Endereço</label>
                        <p className="text-base">{responsavel.endereco_completo}</p>
                      </div>
                      <div>
                    ...
                    <p className="text-muted-foreground">Nenhum responsável financeiro cadastrado.</p>
                  )}
                </div>
              </TabsContent>
                          ...
                          <CardContent className="p-4">
                            <div className="flex justify-between items-center">
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  Tipo: {doc.tipo_arquivo} • Tamanho: {(doc.tamanho_arquivo / 1024 / 1024).toFixed(2)} MB
                                </p>
                                {doc.descricao && (
